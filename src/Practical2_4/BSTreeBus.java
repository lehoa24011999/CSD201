...
public class BSTreeBus {
	...
	LinkedList<BusNode> listNode=new LinkedList<BusNode>();
	public void inOrder(BusNode p){
		if(p==null) return;
		inOrder(p.left);
		visit(p);
		listNode.add(p);
	...
	void copyDele(String x){
		...
		BusNode f,p; // f will be the father of p
		p=root;f=null;
		while(p!=null){
		...
		if(p.left!=null && p.right!=null) {// p has both left and right children
			BusNode rp; // p's key will be replaced by rp's one
			f=null;
			rp = p.left;
			while(rp.right!=null) {
				f=rp; rp = rp.right;
	...
	//	   }
	  ...
	  void balance(ArrayList<BusNode> a, int lower, int upper)
	   {if(lower>upper) return;
	    int middle = (lower + upper)/2;
	    Bus x = ((BusNode) a.get(middle)).info;
	    insert(x);
	    balance(a,lower, middle-1);
	    balance(a,middle+1,upper);
	   }
	  void balance()
	   {ArrayList<BusNode> a = new ArrayList<BusNode>();
	    inOrder2(root,a);
	    int n = a.size();
