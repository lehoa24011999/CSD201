...
public class BSTCustomer {
	...
	int search(String x) {		
		CustomerNode p = root;
		while(p != null) 
			if(x.compareTo(p.info.getcCode())==0) return 1;
			else if (x.compareTo(p.info.getcCode())<0) p = p.left;
			else p  = p.right;
		return 0;
	}
	void insert(Customer x){
		...
		CustomerNode f, p; f=null;p=root;
		while(p!=null){
			if(p.info==x){
			...
			f=p;
			if(x.getcCode().compareTo(p.info.getcCode())<0) p=p.left; else p=p.right;
		}
		if(x.getcCode().compareTo(f.info.getcCode())<0) f.left=q;else f.right=q;
	}
	...
	LinkedList<CustomerNode> listNode=new LinkedList<CustomerNode>();
	public void inOrder(CustomerNode p){
		if(p==null) return;
		inOrder(p.left);
		visit(p);
		listNode.add(p);
	...
	public LinkedList<CustomerNode> getListNode(){
		return listNode;
	}


	/*8.   void postorder(Node p) - recursive postorder traverse of a tree*/
	void postOrder(CustomerNode p){
		if(p==null) return;
		postOrder(p.left);
		postOrder(p.right);
		visit(p);
	...
	void copyDele(String x){
		...
		CustomerNode f,p; // f will be the father of p
		p=root;f=null;
		while(p!=null){
			if(x.compareTo(p.info.getcCode())==0) break;//Found key x
			if(x.compareTo(p.info.getcCode())<0){
				f=p;p=p.left;
			}
			else{
				f=p;p=p.right;
		...
		if(p.left!=null && p.right!=null) {// p has both left and right children
			CustomerNode rp; // p's key will be replaced by rp's one
			f=null;
			rp = p.left;
			while(rp.right!=null) {
				f=rp; rp = rp.right;
	...
	void inOrder2(CustomerNode p, ArrayList<CustomerNode> a)
	{if(p==null) return;
	inOrder2(p.left,a);
	a.add(p);
	inOrder2(p.right,a);
	}
	void balance(ArrayList<CustomerNode> a, int lower, int upper)
	{if(lower>upper) return;
	int middle = (lower + upper)/2;
	Customer x = ((CustomerNode) a.get(middle)).info;
	insert(x);
	balance(a,lower, middle-1);
	balance(a,middle+1,upper);
	}
	void balance()
	{ArrayList<CustomerNode> a = new ArrayList<CustomerNode>();
	inOrder2(root,a);
	int n = a.size();
	BSTCustomer g = new BSTCustomer();
	g.balance(a,0,n-1);
	root=g.root;
	}
