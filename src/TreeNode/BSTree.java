...
//				System.out.println("The key "+x+" already exists, no insertion");
	...
	void copyDele(int x){
		...
		BSTNode f,p; // f will be the father of p
		p=root;f=null;
		while(p!=null){
		...
		if(p.left!=null && p.right!=null) {// p has both left and right children
			BSTNode rp; // p's key will be replaced by rp's one
			f=null;
			rp = p.left;
			while(rp.right!=null) {
				f=rp; rp = rp.right;
